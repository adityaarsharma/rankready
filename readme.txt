...
* Optimization: retention window reduced from 90 to 30 days — covers all admin panel windows and keeps the table lean.
* Optimization: daily prune now enforces a hard 50,000-row cap as a safety valve against runaway crawlers. Runs in existing cron, zero per-request overhead.
...
* Fix: Remove $schema field from /.well-known/agent-skills/index.json that referenced agentskills.io/schema/v1/index.schema.json — that URL returns 404. No public schema exists for this format.

= 0.6.4.3 =
* New: /index.md homepage endpoint (reverted in 0.6.4.4 — non-standard).

= 0.6.4.2 =
* New: Discovery Link headers — every page now sends Link: HTTP headers for llms.txt, llms-full.txt, markdown endpoint, and sitemap for AI agent scanners.
* New: Discovery <link> tags in <head> for llms.txt and llms-full.txt HTML-level discovery.
...
* Fix: /.well-known/ rewrite rules flush correctly in the same request that saves the Agent Skills / API Catalog toggle.
