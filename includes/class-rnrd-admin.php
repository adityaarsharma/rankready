...
class RNRD_Admin {
	...
	/**
	 * Deprecated — admin bar indicator removed in WP.org Free build.
	 *
	 * @deprecated 1.2.0-rc.16
	 */
	public static function admin_bar_pro_indicator( $wp_admin_bar ): void {
		// No-op in WP.org Free build. Retained for backward compatibility only.
	}

	/**
	 * Deprecated — sandbox toggle removed in WP.org Free build.
	 *
	 * @deprecated 1.2.0-rc.16
	 */
	public static function handle_sandbox_toggle(): void {
		// No-op in WP.org Free build.
	}
	...
	public static function enqueue_admin_assets( $hook ): void {
		...
		wp_enqueue_style( 'rnrd-design-tokens', RNRD_URL . 'assets/design-tokens.css', array(), RNRD_VERSION );
		...
		wp_enqueue_style( 'rnrd-admin', RNRD_URL . 'assets/admin.css', array( 'rnrd-design-tokens' ), RNRD_VERSION );
		wp_enqueue_script( 'rnrd-admin', RNRD_URL . 'assets/admin.js', array(), RNRD_VERSION, true );
	...
	public static function register_settings(): void {
		...
		foreach ( $rnrd_mcp_toggles_on as $opt ) {
		...
		foreach ( $rnrd_mcp_toggles_off as $opt ) {
			register_setting( self::LLMS_GROUP, $opt, array(
				'type'              => 'string',
				'sanitize_callback' => array( self::class, 'sanitize_on_off' ),
				'default'           => 'off',
	...
	private static function render_pro_gate( string $feature, string $description = '' ): void {
		...
		<div class="rnrd-pro-gate">
			<span class="dashicons dashicons-lock rnrd-pro-gate__icon" aria-hidden="true"></span>
	...
	private static function render_tab_dashboard(): void {
		...
		<div class="rnrd-whatsnew-banner" style="background:linear-gradient(135deg,#1a1a2e 0%,#16213e 100%);color:#fff;padding:18px 22px;border-radius:8px;margin-bottom:20px;display:flex;align-items:flex-start;gap:16px;">
			<div style="font-size:13px;line-height:1;font-weight:700;letter-spacing:0.5px;color:#9da3c0;text-transform:uppercase;">NEW</div>
			<div style="flex:1;">
				<h3 style="margin:0 0 6px;color:#fff;font-size:16px;"><?php
					/* translators: %s: version number */
					echo esc_html( sprintf( __( 'RankReady %s is here', 'rankready-ai-llm-seo' ), RNRD_VERSION ) );
				?></h3>
				<ul style="margin:0;padding:0;list-style:none;font-size:13px;line-height:1.6;color:#e8e8f0;">
					<li><strong><?php esc_html_e( 'Brand Terms.', 'rankready-ai-llm-seo' ); ?></strong> <?php esc_html_e( 'Add your canonical brand names in AI Crawlers → Brand Terms. Powers entity consistency checks.', 'rankready-ai-llm-seo' ); ?></li>
					<li><strong><?php esc_html_e( 'DeepSeek updated.', 'rankready-ai-llm-seo' ); ?></strong> <?php esc_html_e( 'Default model switched to deepseek-v4-flash — faster and cheaper than the previous default.', 'rankready-ai-llm-seo' ); ?></li>
					<li><strong><?php esc_html_e( 'Bug fixes &amp; improvements.', 'rankready-ai-llm-seo' ); ?></strong> <?php esc_html_e( 'Several stability and accuracy improvements across the plugin.', 'rankready-ai-llm-seo' ); ?></li>
				</ul>
				<div style="margin-top:12px;">
					<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=rankready&rnrd_dismiss_whatsnew=' . RNRD_VERSION ), 'rnrd_dismiss_whatsnew' ) ); ?>" style="color:#9da3c0;text-decoration:none;font-size:13px;"><?php esc_html_e( 'Dismiss', 'rankready-ai-llm-seo' ); ?></a>
				</div>
			</div>
		</div>
		<?php endif; ?>
		...
		<div class="rnrd-card rnrd-tutorial-card" style="margin-bottom:24px;">
			...
			<div style="margin-top:14px;display:flex;gap:10px;flex-wrap:wrap;">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rankready&tab=settings' ) ); ?>" class="button button-secondary"><?php esc_html_e( 'Configure API key →', 'rankready-ai-llm-seo' ); ?></a>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rankready&tab=authority' ) ); ?>" class="button button-secondary"><?php esc_html_e( 'Set up Author Box →', 'rankready-ai-llm-seo' ); ?></a>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rankready&tab=crawlers' ) ); ?>" class="button button-secondary"><?php esc_html_e( 'Enable LLMs.txt →', 'rankready-ai-llm-seo' ); ?></a>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rankready-welcome' ) ); ?>" class="button button-secondary"><?php esc_html_e( 'Re-run setup wizard →', 'rankready-ai-llm-seo' ); ?></a>
			</div>
		</div>
		<?php endif; ?>
		...
		<!-- ── Quick navigation ──────────────────────────────────────────── -->
		<div class="rnrd-info-grid" style="margin-bottom:28px;">
			<div class="rnrd-info-item rnrd-dash-feature">
				<h3><?php esc_html_e( 'Content AI', 'rankready-ai-llm-seo' ); ?></h3>
				<p><?php printf( esc_html__( '%1$d summaries · %2$d FAQ sets on your site.', 'rankready-ai-llm-seo' ), $summary_count, $faq_count ); ?></p>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rankready&tab=content' ) ); ?>" class="rnrd-dash-link"><?php esc_html_e( 'Open →', 'rankready-ai-llm-seo' ); ?></a>
			</div>
			<div class="rnrd-info-item rnrd-dash-feature">
				<h3><?php esc_html_e( 'Authority', 'rankready-ai-llm-seo' ); ?></h3>
				<p><?php esc_html_e( 'Author box, EEAT schema, Article JSON-LD.', 'rankready-ai-llm-seo' ); ?></p>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rankready&tab=authority' ) ); ?>" class="rnrd-dash-link"><?php esc_html_e( 'Open →', 'rankready-ai-llm-seo' ); ?></a>
			</div>
			<div class="rnrd-info-item rnrd-dash-feature">
				<h3><?php esc_html_e( 'AI Crawlers', 'rankready-ai-llm-seo' ); ?></h3>
				<p><?php
					$parts = array();
					...
					echo $parts ? esc_html( implode( ' · ', $parts ) ) : esc_html__( 'LLMs.txt, Markdown, bot controls.', 'rankready-ai-llm-seo' );
				?></p>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rankready&tab=crawlers' ) ); ?>" class="rnrd-dash-link"><?php esc_html_e( 'Open →', 'rankready-ai-llm-seo' ); ?></a>
			</div>
			<div class="rnrd-info-item rnrd-dash-feature">
				<h3><?php esc_html_e( 'Settings', 'rankready-ai-llm-seo' ); ?></h3>
				<p><?php
					if ( $api_set ) {
						...
						echo '<strong style="color:#d63638;">' . esc_html__( 'AI provider key required', 'rankready-ai-llm-seo' ) . '</strong>';
					}
				?></p>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rankready&tab=settings' ) ); ?>" class="rnrd-dash-link"><?php esc_html_e( 'Open →', 'rankready-ai-llm-seo' ); ?></a>
			</div>
			<div class="rnrd-info-item rnrd-dash-feature">
				<h3><?php esc_html_e( 'Tools', 'rankready-ai-llm-seo' ); ?></h3>
				<p><?php esc_html_e( 'Health check, bulk actions, content signals.', 'rankready-ai-llm-seo' ); ?></p>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rankready&tab=advanced' ) ); ?>" class="rnrd-dash-link"><?php esc_html_e( 'Open →', 'rankready-ai-llm-seo' ); ?></a>
			</div>
			<div class="rnrd-info-item rnrd-dash-feature">
				<h3><?php esc_html_e( 'Plugin Info', 'rankready-ai-llm-seo' ); ?></h3>
				<p>v<?php echo esc_html( RNRD_VERSION ); ?> &middot; <a href="https://posimyth.com" target="_blank" rel="noopener" style="color:#646970;text-decoration:none;"><?php esc_html_e( 'by POSIMYTH Innovations', 'rankready-ai-llm-seo' ); ?></a></p>
				<a href="<?php echo esc_url( admin_url( 'update-core.php' ) ); ?>" class="rnrd-dash-link"><?php esc_html_e( 'Check for updates', 'rankready-ai-llm-seo' ); ?></a>
			</div>
		</div>
		...
		<div class="rnrd-card" style="margin-bottom:24px;">
			<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:6px;">
				...
				<span style="background:#2271b1;color:#fff;font-size:11px;font-weight:700;padding:2px 8px;border-radius:3px;letter-spacing:.5px;"><?php esc_html_e( 'BETA BUILD', 'rankready-ai-llm-seo' ); ?></span>
			</div>
	...
	private static function render_card_scorecard(): void {
		...
		<div class="rnrd-card rnrd-scorecard" style="margin-bottom:24px;">
			...
			<div class="rnrd-scorecard-progress" style="position:relative;height:10px;background:#e5e5e7;border-radius:6px;overflow:hidden;margin:10px 0 6px;">
				<div class="rnrd-scorecard-bar" style="height:100%;width:<?php echo (int) $pct; ?>%;background:linear-gradient(90deg,#00a32a 0%,#2271b1 100%);transition:width 0.4s ease;"></div>
			</div>
	...
	private static function render_tab_content_ai(): void {
		// Top capabilities banner removed in rc.16 — generation is unlimited
		// in Free; Auto-generate / Bulk live as locked Pro-style cards below
		// with the standard COMING SOON overlay applied via CSS.
		?>
		<?php settings_errors(); ?>
		...
		<form method="post" action="options.php" novalidate="novalidate">
			...
			<div class="rnrd-section-header">
				<h2 class="rnrd-section-title"><?php esc_html_e( 'AI Summary', 'rankready-ai-llm-seo' ); ?></h2>
				<p class="rnrd-section-desc"><?php esc_html_e( 'Generate key takeaways from post content via your active AI provider. Display via block, widget, or auto-inject.', 'rankready-ai-llm-seo' ); ?></p>
			</div>
			<?php self::render_tab_summary(); ?>

			<div class="rnrd-section-divider"></div>

			<div class="rnrd-section-header">
				<h2 class="rnrd-section-title"><?php esc_html_e( 'FAQ Generator', 'rankready-ai-llm-seo' ); ?></h2>
				<p class="rnrd-section-desc"><?php esc_html_e( 'Generate FAQPage schema and an expandable FAQ section using DataForSEO question discovery + your active AI provider for answers.', 'rankready-ai-llm-seo' ); ?></p>
			</div>
			<?php self::render_tab_faq(); ?>

			<?php submit_button( __( 'Save Content AI Settings', 'rankready-ai-llm-seo' ) ); ?>
		</form>
	...
	private static function render_tab_authority(): void {
		...
		<form method="post" action="options.php" novalidate="novalidate">
			...
			<div class="rnrd-section-header">
				<h2 class="rnrd-section-title"><?php esc_html_e( 'Author Box', 'rankready-ai-llm-seo' ); ?></h2>
				<p class="rnrd-section-desc"><?php esc_html_e( 'EEAT-optimized author bio with Person JSON-LD schema. Smart-merges with Rank Math, Yoast, and other SEO plugins.', 'rankready-ai-llm-seo' ); ?></p>
			</div>
			...
			<div class="rnrd-section-header">
				<h2 class="rnrd-section-title"><?php esc_html_e( 'Schema Automation', 'rankready-ai-llm-seo' ); ?></h2>
				<p class="rnrd-section-desc"><?php esc_html_e( 'FAQPage, HowTo, ItemList, and Article JSON-LD — auto-detected from your content structure.', 'rankready-ai-llm-seo' ); ?></p>
			</div>
			<?php self::render_tab_schema(); ?>

			<?php submit_button( __( 'Save Authority Settings', 'rankready-ai-llm-seo' ) ); ?>
		</form>
	...
	private static function render_tab_insights(): void {
		...
		$is_pro = function_exists( 'rnrd_is_pro' ) && rnrd_is_pro();
		$sub_tabs = array(
			'bot-activity' => __( 'Bot Activity', 'rankready-ai-llm-seo' ),
			'citation'     => __( 'AI Citation Candidates', 'rankready-ai-llm-seo' ),
			'referral'     => __( 'AI Referral Traffic', 'rankready-ai-llm-seo' ),
			'freshness'    => __( 'Content Freshness', 'rankready-ai-llm-seo' ),
			'mentions'     => __( 'AI Mention Tracker', 'rankready-ai-llm-seo' ) . ' (Coming Soon)',
		);
		...
		?>
		<style>
			.rnrd-insights-funnel { display:grid; grid-template-columns:repeat(auto-fit,minmax(140px,1fr)); gap:8px; margin:0 0 20px; padding:14px; background:linear-gradient(135deg,var(--rnrd-color-surface,#fff) 0%,var(--rnrd-color-brand-soft,#f0f6fc) 100%); border:1px solid var(--rnrd-color-border,#c3c4c7); border-radius:var(--rnrd-radius-md,6px); }
			.rnrd-insights-funnel__step { padding:8px 12px; border-radius:var(--rnrd-radius-md,6px); background:rgba(255,255,255,0.7); font-size:11px; line-height:1.45; }
			.rnrd-insights-funnel__step strong { display:block; font-size:12px; color:var(--rnrd-color-ink,#1d2327); margin-bottom:2px; }
			.rnrd-insights-funnel__arrow { display:flex; align-items:center; justify-content:center; color:var(--rnrd-color-text-muted,#646970); font-size:18px; }
			.rnrd-insights-subnav { display:flex; gap:0; border-bottom:1px solid var(--rnrd-color-border,#c3c4c7); margin:0 0 20px; }
			.rnrd-insights-subnav a { padding:10px 16px; text-decoration:none; color:var(--rnrd-color-text-muted,#646970); font-size:13px; font-weight:500; border-bottom:2px solid transparent; }
			.rnrd-insights-subnav a.is-active { color:var(--rnrd-color-brand,#2271b1); border-bottom-color:var(--rnrd-color-brand,#2271b1); font-weight:600; }
			.rnrd-insights-subnav a:hover { background:var(--rnrd-color-surface-2,#f6f7f7); }
			.rnrd-insights-section-header { margin:0 0 16px; padding:12px 16px; background:var(--rnrd-color-surface-2,#f6f7f7); border-left:4px solid var(--rnrd-color-brand,#2271b1); border-radius:0 var(--rnrd-radius-md,6px) var(--rnrd-radius-md,6px) 0; }
			.rnrd-insights-section-header h3 { margin:0 0 4px; font-size:15px; }
			.rnrd-insights-section-header p { margin:0; font-size:12px; color:var(--rnrd-color-text-muted,#646970); line-height:1.5; }
			.rnrd-insights-section-header--training { border-left-color:var(--rnrd-color-info,#3582c4); }
			.rnrd-insights-section-header--citation { border-left-color:var(--rnrd-color-success,#00a32a); }
			.rnrd-insights-section-header--referral { border-left-color:var(--rnrd-color-warning,#dba617); }
		</style>

		<?php settings_errors(); ?>

		<!-- Plain-English "what am I looking at?" intro — shown above the funnel diagram -->
		<div class="rnrd-card" style="margin:0 0 16px;padding:14px 18px;background:#fff;border-left:4px solid #2271b1;">
			<h2 style="margin:0 0 6px;font-size:14px;color:#1d2327;"><?php esc_html_e( 'What is the Insights tab?', 'rankready-ai-llm-seo' ); ?></h2>
			<p style="margin:0;font-size:13px;line-height:1.55;color:#3c434a;">
				<?php esc_html_e( 'This tab tracks the four stages of AI search traffic to your site. RankReady detects every known AI crawler that reads your llms.txt, /*.md, or homepage endpoints — then classifies each hit by intent (training vs live citation) so you can see which bots are bringing AI answers back to your content. Counts start at zero on a fresh install. First citation hits typically arrive within 2–6 weeks of enabling RankReady, depending on how often your pages already rank for queries users ask AI assistants.', 'rankready-ai-llm-seo' ); ?>
			</p>
		</div>

		<!-- Funnel explainer — visualises the 3 stages so users know which sub-tab to use -->
		<div class="rnrd-insights-funnel">
			<div class="rnrd-insights-funnel__step">
				<strong>① Training crawl</strong>
				<?php esc_html_e( 'GPTBot / ClaudeBot / Google-Extended index your site for future training.', 'rankready-ai-llm-seo' ); ?>
			</div>
			<div class="rnrd-insights-funnel__arrow" aria-hidden="true">→</div>
			<div class="rnrd-insights-funnel__step">
				<strong>② Citation crawl</strong>
				<?php esc_html_e( 'ChatGPT-User / PerplexityBot / OAI-SearchBot fetch your page to answer a live user query.', 'rankready-ai-llm-seo' ); ?>
			</div>
			<div class="rnrd-insights-funnel__arrow" aria-hidden="true">→</div>
			<div class="rnrd-insights-funnel__step">
				<strong>③ Referral click</strong>
				<?php esc_html_e( 'User reads the AI answer, clicks through to your site.', 'rankready-ai-llm-seo' ); ?>
			</div>
			<div class="rnrd-insights-funnel__arrow" aria-hidden="true">→</div>
			<div class="rnrd-insights-funnel__step">
				<strong>④ Conversion</strong>
				<?php esc_html_e( '23× higher than non-cited visits (Seer Interactive 2026).', 'rankready-ai-llm-seo' ); ?>
			</div>
		</div>
	...
	private static function render_insights_mention_tracker(): void {
		?>
		<div class="rnrd-insights-section-header" style="border-left-color:#8b5cf6;">
			<h3><?php esc_html_e( 'AI Mention Tracker', 'rankready-ai-llm-seo' ); ?>
				<span style="background:#8b5cf6;color:#fff;font-size:10px;font-weight:700;padding:2px 8px;border-radius:3px;letter-spacing:.5px;margin-left:8px;"><?php esc_html_e( 'COMING SOON', 'rankready-ai-llm-seo' ); ?></span>
			</h3>
			<p><?php esc_html_e( 'Actively monitor brand mentions across ChatGPT, Perplexity, Claude, and Gemini. Ask each LLM your target queries weekly and track position changes over time.', 'rankready-ai-llm-seo' ); ?></p>
		</div>

		<div class="rnrd-card" style="background:linear-gradient(135deg,#f5f3ff 0%,#fff 100%);border:1px solid #ddd6fe;padding:32px;text-align:center;">
			<div style="display:inline-block;padding:4px 12px;background:#8b5cf6;color:#fff;font-size:11px;font-weight:700;letter-spacing:0.5px;border-radius:3px;margin-bottom:12px;text-transform:uppercase;">Coming Soon</div>
			<h3 style="margin:0 0 8px;font-size:18px;color:#1d2327;"><?php esc_html_e( 'AI Mention Tracker', 'rankready-ai-llm-seo' ); ?></h3>
			<p style="margin:0 0 20px;color:#646970;max-width:540px;margin-left:auto;margin-right:auto;font-size:13px;line-height:1.6;">
				<?php esc_html_e( 'Track brand mentions across major LLMs week-over-week, with position history and alerts. We are building it now. No action required from you today — the feature will appear automatically when it ships.', 'rankready-ai-llm-seo' ); ?>
			</p>

			<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;max-width:760px;margin:0 auto 0;text-align:left;">
				<div style="padding:14px;background:#fff;border-radius:8px;border:1px solid #ede9fe;">
					<strong style="font-size:13px;display:block;margin-bottom:4px;"><?php esc_html_e( 'Track target queries', 'rankready-ai-llm-seo' ); ?></strong>
					<span style="font-size:12px;color:#646970;line-height:1.5;"><?php esc_html_e( 'See whether AI answers cite your pages.', 'rankready-ai-llm-seo' ); ?></span>
				</div>
				<div style="padding:14px;background:#fff;border-radius:8px;border:1px solid #ede9fe;">
					<strong style="font-size:13px;display:block;margin-bottom:4px;"><?php esc_html_e( 'Across all major LLMs', 'rankready-ai-llm-seo' ); ?></strong>
					<span style="font-size:12px;color:#646970;line-height:1.5;">ChatGPT · Claude · Gemini · DeepSeek</span>
				</div>
				<div style="padding:14px;background:#fff;border-radius:8px;border:1px solid #ede9fe;">
					<strong style="font-size:13px;display:block;margin-bottom:4px;"><?php esc_html_e( 'Position history', 'rankready-ai-llm-seo' ); ?></strong>
					<span style="font-size:12px;color:#646970;line-height:1.5;"><?php esc_html_e( 'Weekly snapshot of where you rank.', 'rankready-ai-llm-seo' ); ?></span>
				</div>
				<div style="padding:14px;background:#fff;border-radius:8px;border:1px solid #ede9fe;">
					<strong style="font-size:13px;display:block;margin-bottom:4px;"><?php esc_html_e( 'Competitor comparison', 'rankready-ai-llm-seo' ); ?></strong>
					<span style="font-size:12px;color:#646970;line-height:1.5;"><?php esc_html_e( 'See who AI cites instead of you.', 'rankready-ai-llm-seo' ); ?></span>
				</div>
			</div>
		</div>
		<?php
	...
	private static function render_insights_bot_activity(): void {
		$total_30d     = RNRD_Crawler_Log::get_total( 30 );
		$citation_hits = RNRD_Crawler_Log::get_citation_hits_total( 30 );
		$training_hits = RNRD_Crawler_Log::get_training_hits_total( 30 );
		$unique_pages  = RNRD_Crawler_Log::get_unique_pages( 30 );
		$bot_stats     = RNRD_Crawler_Log::get_bot_stats( 30 );
		?>
		<div class="rnrd-insights-section-header rnrd-insights-section-header--citation">
			<h3><?php esc_html_e( 'Citation crawl — live AI answer fetches', 'rankready-ai-llm-seo' ); ?></h3>
			<p><?php esc_html_e( 'OAI-SearchBot, ChatGPT-User, PerplexityBot, Claude-Web, DuckAssistBot. Every hit ≈ one live AI response that retrieved your content as a source. Optimise these first — they\'re winning right now.', 'rankready-ai-llm-seo' ); ?></p>
		</div>
		<div class="rnrd-card" style="margin-bottom:24px;background:linear-gradient(135deg,var(--rnrd-color-success-bg,#d1ecdf) 0%,var(--rnrd-color-brand-soft,#f0f6fc) 100%);border:1px solid var(--rnrd-color-success,#00a32a);">
			<div style="display:flex;align-items:baseline;gap:12px;">
				<div style="font-size:42px;font-weight:700;color:var(--rnrd-color-success-text,#0a6c39);line-height:1;"><?php echo esc_html( number_format( $citation_hits ) ); ?></div>
				<div style="font-size:13px;color:var(--rnrd-color-success-text,#0a6c39);">
					<?php esc_html_e( 'citation hits — last 30 days', 'rankready-ai-llm-seo' ); ?><br />
					<span style="opacity:0.75;font-size:11px;"><?php esc_html_e( 'fetched by ', 'rankready-ai-llm-seo' ); ?><strong><?php
						$cit_bots = array_filter( $bot_stats, function ( $r ) { return 'citation' === RNRD_Crawler_Log::bot_intent( $r['bot_name'] ); } );
						echo esc_html( count( $cit_bots ) );
					?></strong> <?php esc_html_e( 'unique citation bots', 'rankready-ai-llm-seo' ); ?></span>
				</div>
			</div>
		</div>

		<div class="rnrd-insights-section-header rnrd-insights-section-header--training">
			<h3><?php esc_html_e( 'Training crawl — for future model training', 'rankready-ai-llm-seo' ); ?></h3>
			<p><?php esc_html_e( 'GPTBot, ClaudeBot, Google-Extended, Bytespider, CCBot. Content ingested today may surface in model weights months later. Higher volume than citation crawl by 10-100×; lower immediate ROI.', 'rankready-ai-llm-seo' ); ?></p>
		</div>
		<div class="rnrd-card" style="margin-bottom:24px;background:linear-gradient(135deg,var(--rnrd-color-info-bg,#e5f1f9) 0%,var(--rnrd-color-brand-soft,#f0f6fc) 100%);border:1px solid var(--rnrd-color-info,#3582c4);">
			<div style="display:flex;align-items:baseline;gap:12px;">
				<div style="font-size:42px;font-weight:700;color:var(--rnrd-color-info-text,#135e96);line-height:1;"><?php echo esc_html( number_format( $training_hits ) ); ?></div>
				<div style="font-size:13px;color:var(--rnrd-color-info-text,#135e96);">
					<?php esc_html_e( 'training hits — last 30 days', 'rankready-ai-llm-seo' ); ?><br />
					<span style="opacity:0.75;font-size:11px;"><?php
						$train_bots = array_filter( $bot_stats, function ( $r ) { return 'training' === RNRD_Crawler_Log::bot_intent( $r['bot_name'] ); } );
						echo esc_html( count( $train_bots ) );
					?></strong> <?php esc_html_e( 'unique training bots', 'rankready-ai-llm-seo' ); ?></span>
				</div>
			</div>
		</div>

		<div class="rnrd-card" style="margin-bottom:24px;background:var(--rnrd-color-surface-2,#f6f7f7);">
			<h4 style="margin:0 0 10px;font-size:13px;color:var(--rnrd-color-text-muted,#646970);text-transform:uppercase;letter-spacing:0.04em;">
				<?php esc_html_e( 'Combined activity', 'rankready-ai-llm-seo' ); ?>
			</h4>
			<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;">
				<div style="text-align:center;padding:10px;background:#fff;border-radius:6px;">
					...
					<div style="font-size:11px;color:var(--rnrd-color-text-muted,#646970);"><?php esc_html_e( 'Total hits — 30 days', 'rankready-ai-llm-seo' ); ?></div>
				</div>
				<div style="text-align:center;padding:10px;background:#fff;border-radius:6px;">
					<div style="font-size:24px;font-weight:700;"><?php echo esc_html( number_format( $unique_pages ) ); ?></div>
					<div style="font-size:11px;color:var(--rnrd-color-text-muted,#646970);"><?php esc_html_e( 'Unique pages read', 'rankready-ai-llm-seo' ); ?></div>
				</div>
				<div style="text-align:center;padding:10px;background:#fff;border-radius:6px;">
					<div style="font-size:24px;font-weight:700;"><?php echo esc_html( count( $bot_stats ) ); ?></div>
					<div style="font-size:11px;color:var(--rnrd-color-text-muted,#646970);"><?php esc_html_e( 'Unique bots', 'rankready-ai-llm-seo' ); ?></div>
				</div>
			</div>
		</div>

		<?php if ( empty( $bot_stats ) ) : ?>
			<p style="padding:24px;text-align:center;color:var(--rnrd-color-text-muted,#646970);font-style:italic;background:var(--rnrd-color-surface-2,#f6f7f7);border-radius:var(--rnrd-radius-md,6px);">
				<?php esc_html_e( 'No AI bot visits recorded yet. Once GPTBot, ChatGPT-User, PerplexityBot, ClaudeBot, or any of the other 20 tracked AI crawlers hits your llms.txt, .md, or homepage endpoints, activity will appear here automatically. Typical first hit: 24–72 hours after enabling llms.txt.', 'rankready-ai-llm-seo' ); ?>
			</p>
		<?php else : ?>
			<p class="description" style="margin-top:8px;">
				<?php esc_html_e( 'Full breakdown by bot, with intent badges and top-page drill-down, lives on the AI Crawlers tab → AI Crawler Access Log card (until beta.8 consolidates it here).', 'rankready-ai-llm-seo' ); ?>
			</p>
		<?php endif; ?>
		<?php
	}
	...
	private static function render_insights_citation(): void {
		...
		?>
		<div class="rnrd-insights-section-header rnrd-insights-section-header--citation">
			<h3><?php esc_html_e( 'AI Citation Candidates — last 30 days', 'rankready-ai-llm-seo' ); ?></h3>
			<p><?php esc_html_e( 'Pages most fetched by citation-intent bots. Each hit is a live AI answer that retrieved this page as a source. Optimise these first — they\'re already winning. Refresh their content, expand the FAQ, set max-snippet to allow full quoting.', 'rankready-ai-llm-seo' ); ?></p>
		</div>

		<?php if ( empty( $citation_pages ) ) : ?>
			<div class="rnrd-card" style="text-align:center;padding:32px;color:var(--rnrd-color-text-muted,#646970);">
				<p style="margin:0 0 6px;font-size:14px;font-weight:600;color:var(--rnrd-color-ink,#1d2327);">
					<?php esc_html_e( 'No citation bot hits yet', 'rankready-ai-llm-seo' ); ?>
				</p>
				<p style="margin:0 0 8px;font-size:12px;line-height:1.6;max-width:520px;margin-left:auto;margin-right:auto;">
					<?php esc_html_e( 'ChatGPT-User, OAI-SearchBot, PerplexityBot, Claude-Web, and DuckAssistBot only fetch your pages when a real user asks the AI something your content might answer.', 'rankready-ai-llm-seo' ); ?>
				</p>
				<p style="margin:0;font-size:12px;line-height:1.6;max-width:520px;margin-left:auto;margin-right:auto;">
					<?php esc_html_e( 'Typical first citation: 2–6 weeks after enabling RankReady. Speed it up by adding FAQs to your top 10 posts and keeping content fresh.', 'rankready-ai-llm-seo' ); ?>
				</p>
			</div>
		<?php else : ?>
			<table class="wp-list-table widefat striped">
				<thead>
					<tr>
						<th style="width:55%;"><?php esc_html_e( 'Page', 'rankready-ai-llm-seo' ); ?></th>
						<th style="width:80px;text-align:center;"><?php esc_html_e( 'Hits', 'rankready-ai-llm-seo' ); ?></th>
						<th style="width:80px;text-align:center;"><?php esc_html_e( 'Bots', 'rankready-ai-llm-seo' ); ?></th>
						<th style="width:150px;"><?php esc_html_e( 'Last fetched', 'rankready-ai-llm-seo' ); ?></th>
						<th style="width:80px;"><?php esc_html_e( 'Edit', 'rankready-ai-llm-seo' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php foreach ( $citation_pages as $cp ) :
						...
						<tr>
							<td>
								...
								<?php if ( $cp_type ) : ?>
									<span style="display:inline-block;margin-left:6px;padding:1px 6px;background:var(--rnrd-color-surface-3,#f0f0f1);border-radius:3px;font-size:10px;color:var(--rnrd-color-text-muted,#646970);"><?php echo esc_html( $cp_type ); ?></span>
								<?php endif; ?>
								<br />
								<code style="font-size:10px;color:var(--rnrd-color-text-muted,#646970);"><?php echo esc_html( $cp_path ); ?></code>
							</td>
							<td style="text-align:center;font-weight:700;color:var(--rnrd-color-success-text,#0a6c39);"><?php echo esc_html( number_format( $cp_hits ) ); ?></td>
							<td style="text-align:center;"><?php echo esc_html( $cp_bots ); ?></td>
							<td style="font-size:11px;color:var(--rnrd-color-text-muted,#646970);"><?php echo $cp_seen_ts ? esc_html( human_time_diff( $cp_seen_ts ) . ' ' . __( 'ago', 'rankready-ai-llm-seo' ) ) : '—'; ?></td>
							<td>
								<?php if ( $cp_post_id > 0 ) : ?>
	...
	private static function render_insights_referral(): void {
		...
		?>
		<div class="rnrd-insights-section-header rnrd-insights-section-header--referral">
			<h3>👥 <?php esc_html_e( 'AI Referral Traffic — outbound (users clicking from AI)', 'rankready-ai-llm-seo' ); ?></h3>
			<p><strong><?php esc_html_e( 'Different from bot hits above.', 'rankready-ai-llm-seo' ); ?></strong> <?php esc_html_e( 'This measures real humans who read an AI answer and clicked through to your site. Counted via Referer header — 100% server-side, no third-party scripts.', 'rankready-ai-llm-seo' ); ?></p>
		</div>

		<div class="rnrd-card" style="margin-bottom:24px;">
			<div style="display:flex;align-items:baseline;gap:12px;margin-bottom:16px;">
				<div style="font-size:42px;font-weight:700;color:var(--rnrd-color-warning-text,#674c00);line-height:1;"><?php echo esc_html( number_format( $total ) ); ?></div>
				<div style="font-size:13px;color:var(--rnrd-color-text-muted,#646970);">
					<?php esc_html_e( 'AI-sourced visits — last 30 days', 'rankready-ai-llm-seo' ); ?><br />
					<span style="opacity:0.75;font-size:11px;"><?php esc_html_e( 'across ChatGPT, Perplexity, Gemini, Claude, Copilot', 'rankready-ai-llm-seo' ); ?></span>
				</div>
			</div>
			...
			<?php if ( $total > 0 ) : ?>
				<ul style="margin:0;padding:0;list-style:none;">
					...
					$max = max( $counts ) ?: 1;
					$colours = array(
						'chatgpt'    => 'var(--rnrd-source-chatgpt,#10a37f)',
						'perplexity' => 'var(--rnrd-source-perplexity,#21808d)',
						'gemini'     => 'var(--rnrd-source-gemini,#4285f4)',
						'claude'     => 'var(--rnrd-source-claude,#cc785c)',
						'copilot'    => 'var(--rnrd-source-copilot,#0078d4)',
					);
					foreach ( $counts as $source => $count ) :
						...
						<li style="margin-bottom:8px;">
							<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px;">
								<span><?php echo esc_html( class_exists( 'RNRD_AI_Referral' ) ? RNRD_AI_Referral::source_label( $source ) : ucfirst( $source ) ); ?></span>
				...
				<p style="margin-top:14px;padding:10px;background:var(--rnrd-color-success-bg,#d1ecdf);border-radius:6px;font-size:12px;color:var(--rnrd-color-success-text,#0a6c39);">
					<strong><?php esc_html_e( '23× conversion lift', 'rankready-ai-llm-seo' ); ?></strong> &mdash;
					<?php esc_html_e( 'cited brands convert at 23× the rate of non-cited competitors (Seer Interactive 2026). Each visitor here arrived via an AI citation.', 'rankready-ai-llm-seo' ); ?>
				</p>
			<?php else : ?>
				<p style="color:var(--rnrd-color-text-muted,#646970);font-style:italic;font-size:13px;margin:0;">
					<?php esc_html_e( 'No AI-sourced visits yet. Tracking is live — counters fill in as ChatGPT, Perplexity, Gemini, Claude, or Copilot send their first visitor. Typical first citation: 2–6 weeks after enabling RankReady.', 'rankready-ai-llm-seo' ); ?>
				</p>
			<?php endif; ?>
		</div>

		<details>
			<summary style="cursor:pointer;font-size:12px;color:var(--rnrd-color-text-muted,#646970);"><?php esc_html_e( 'How is this different from Bot Activity?', 'rankready-ai-llm-seo' ); ?></summary>
			<div style="padding:12px;background:var(--rnrd-color-surface-2,#f6f7f7);border-radius:6px;margin-top:8px;font-size:12px;line-height:1.6;">
				<p style="margin:0 0 6px;"><strong><?php esc_html_e( 'Bot Activity', 'rankready-ai-llm-seo' ); ?></strong> = <?php esc_html_e( 'AI engines crawling your site (server logs). No human involved. Measures whether your site is in the AI\'s index/retrieval pool.', 'rankready-ai-llm-seo' ); ?></p>
				<p style="margin:0;"><strong><?php esc_html_e( 'AI Referral Traffic', 'rankready-ai-llm-seo' ); ?></strong> = <?php esc_html_e( 'Humans who read the AI\'s answer, saw your site listed as a source, and clicked through. Measures whether the AI is sending real users your way.', 'rankready-ai-llm-seo' ); ?></p>
			</div>
		</details>
		<?php
	}
	...
	private static function render_insights_freshness(): void {
		// Merged in rc.16 — the standalone intro header is gone; the scan
		// card below now carries both the rationale paragraph and the tool.
		self::render_card_freshness_alerts();

		if ( class_exists( 'RNRD_Freshness' ) ) {
			RNRD_Freshness::render_widget();
		} else {
			echo '<p>' . esc_html__( 'Freshness module unavailable.', 'rankready-ai-llm-seo' ) . '</p>';
		}
	}
	...
	private static function render_tab_advanced(): void {
		...
		?>
		<div class="rnrd-section-header">
			<h2 class="rnrd-section-title"><?php esc_html_e( 'Tools', 'rankready-ai-llm-seo' ); ?></h2>
			<p class="rnrd-section-desc"><?php esc_html_e( 'Bulk operations, diagnostics, error log, API usage, and data retention.', 'rankready-ai-llm-seo' ); ?></p>
		</div>
	...
	private static function render_tab_api(): void {
		...
		<form method="post" action="options.php" novalidate="novalidate">
			...
			<div class="rnrd-card">
				...
				<p class="rnrd-card-goal"><?php esc_html_e( 'Pick the LLM that powers Summary + FAQ. Switch any time. Only the selected provider needs a key.', 'rankready-ai-llm-seo' ); ?></p>
				<p class="rnrd-card-desc"><?php esc_html_e( 'Pick which AI powers Summary + FAQ generation. Switch any time. Only the selected provider needs an API key — the others stay dormant.', 'rankready-ai-llm-seo' ); ?></p>

				<table class="form-table rnrd-form-table">
					<tr>
						<th scope="row"><?php esc_html_e( 'Active provider', 'rankready-ai-llm-seo' ); ?></th>
						<td>
							<fieldset id="rnrd-llm-provider-picker">
								<?php
								$providers = array(
			...
			$render_verify_row = function ( $provider_id ) {
				...
				<p style="margin-top:8px;">
					...
					<span data-rnrd-verify-status="<?php echo esc_attr( $provider_id ); ?>" style="margin-left:10px;font-size:13px;display:none;"></span>
				</p>
				<?php
			};
			?>
			...
			<!-- Merged in rc.6: 4 provider detail cards → 1 outer card.
			     Inner divs keep [data-rnrd-provider="..."] so the existing
			     JS visibility selector (lines below) continues to toggle them.
			     Outer .rnrd-provider-card class dropped → .rnrd-provider-card-inner. -->
			<div class="rnrd-card">
				<h2 class="rnrd-card-title"><?php esc_html_e( 'Provider Configuration', 'rankready-ai-llm-seo' ); ?></h2>
				<p class="rnrd-card-goal"><?php esc_html_e( 'Pick the LLM that powers Summary + FAQ. Switch any time. Only the selected provider needs a key.', 'rankready-ai-llm-seo' ); ?></p>
				<p class="rnrd-card-desc"><?php esc_html_e( 'API key and model selection for the active provider. Switching providers above swaps this panel.', 'rankready-ai-llm-seo' ); ?></p>
			...
			</div><!-- /.rnrd-card "Provider Configuration" -->

			<script>
			(function(){
				// Hide non-selected provider cards on radio change.
				// Selector matches the 4 .rnrd-provider-card-inner divs created in rc.6.
				const radios = document.querySelectorAll('[data-rnrd-provider-radio]');
				const cards  = document.querySelectorAll('[data-rnrd-provider]');
				const sync = () => {
					const sel = document.querySelector('[data-rnrd-provider-radio]:checked');
					if (!sel) return;
					cards.forEach(c => { c.style.display = (c.dataset.rnrdProvider === sel.value) ? '' : 'none'; });
				};
					...
					<tr>
						...
						<td>
							...
							<p style="margin-top:8px;">
								...
								<span id="rnrd-verify-dfs-status" style="margin-left:10px;font-size:13px;display:none;"></span>
							</p>
						</td>
					</tr>
				</table>
			...
			<?php submit_button( __( 'Save Settings', 'rankready-ai-llm-seo' ) ); ?>
		</form>

		<!-- Connection Status -->
		<div class="rnrd-card rnrd-card--subtle">
			<h3 class="rnrd-card-title" style="font-size:14px;"><?php esc_html_e( 'Status', 'rankready-ai-llm-seo' ); ?></h3>
			<p class="rnrd-card-goal"><?php esc_html_e( 'Live verification — AI key, DataForSEO key, and the active model in one glance.', 'rankready-ai-llm-seo' ); ?></p>
			<div class="rnrd-stats-row" style="margin-top:12px;">
				<div class="rnrd-stat">
					<span class="rnrd-stat-number"><?php echo RNRD_LLM::active_provider_ready() ? '&#10003;' : '&#10007;'; ?></span>
					<span class="rnrd-stat-label"><?php
						echo esc_html( sprintf(
							/* translators: %s: provider name */
							__( '%s Key', 'rankready-ai-llm-seo' ),
							RNRD_LLM::get_provider_label( RNRD_LLM::get_active_provider() )
						) );
					?></span>
				</div>
				<div class="rnrd-stat">
					<span class="rnrd-stat-number"><?php echo ! empty( get_option( RNRD_OPT_DFS_LOGIN, '' ) ) && ! empty( get_option( RNRD_OPT_DFS_PASSWORD, '' ) ) ? '&#10003;' : '&#10007;'; ?></span>
					<span class="rnrd-stat-label"><?php esc_html_e( 'DataForSEO', 'rankready-ai-llm-seo' ); ?></span>
				</div>
				<div class="rnrd-stat">
					<span class="rnrd-stat-number" style="font-size:14px;"><?php echo esc_html( RNRD_LLM::get_model( RNRD_LLM::get_active_provider() ) ); ?></span>
					<span class="rnrd-stat-label"><?php esc_html_e( 'Active Model', 'rankready-ai-llm-seo' ); ?></span>
				</div>
			</div>
		</div>
	...
	private static function render_tab_summary(): void {
		?>
			...
			<div class="rnrd-card">
				...
				<p class="rnrd-card-goal"><?php esc_html_e( 'Auto-generate AI-readable Key Takeaways for every post. AI engines quote these directly.', 'rankready-ai-llm-seo' ); ?></p>
				...
				<table class="form-table rnrd-form-table">
					...
					<tr>
						...
						<td>
							<?php $auto_gen = (string) get_option( RNRD_OPT_AUTO_GENERATE, 'off' ); ?>
							<label>
				...
				<?php self::render_pro_gate(
					__( 'Auto-Generate Summary on Publish', 'rankready-ai-llm-seo' ),
					__( 'Auto-generate the AI Summary the moment you hit Publish. Coming Soon.', 'rankready-ai-llm-seo' )
				); ?>
				<?php endif; ?>
				...
				<table class="form-table rnrd-form-table">
					<tr>
						...
						<td>
							...
							<label>
								<input type="radio" name="<?php echo esc_attr( RNRD_OPT_AUTO_DISPLAY ); ?>" value="on" <?php checked( $auto_display, 'on' ); ?> />
								<?php esc_html_e( 'On — Automatically inject summary into post content', 'rankready-ai-llm-seo' ); ?>
							</label><br/>
							<label>
								<input type="radio" name="<?php echo esc_attr( RNRD_OPT_AUTO_DISPLAY ); ?>" value="off" <?php checked( $auto_display, 'off' ); ?> />
								<?php esc_html_e( 'Off — Only show via block, widget, or shortcode', 'rankready-ai-llm-seo' ); ?>
							</label>
						</td>
					</tr>
					<tr>
	...
	private static function render_tab_author(): void {
		...
		?>
			...
			<div class="rnrd-card">
				...
				<p class="rnrd-card-goal"><?php esc_html_e( 'Show real people behind your content. E-E-A-T signals AI engines check before citing your site.', 'rankready-ai-llm-seo' ); ?></p>
				<?php if ( $seo_plugin ) : ?>
				...
				<table class="form-table rnrd-form-table">
						...
						<td>
							...
							<?php if ( ! empty( $seo_plugin ) ) : ?>
								<p class="description" style="margin-top:4px;color:#666;">
									<?php echo esc_html( sprintf( __( 'Disabled — %s handles Article schema.', 'rankready-ai-llm-seo' ), $seo_plugin ) ); ?>
								</p>
						...
						<td>
							...
							<?php if ( ! empty( $seo_plugin ) ) : ?>
								<p class="description" style="color:#666;">
									<?php echo esc_html( sprintf( __( 'Auto-skips when a %s FAQ block is present in the post content to prevent duplicates.', 'rankready-ai-llm-seo' ), $seo_plugin ) ); ?>
								</p>
							<?php endif; ?>
						</td>
					</tr>
					...
					<tr>
						...
						<td>
							...
							<?php if ( ! empty( $seo_plugin ) ) : ?>
								<p class="description" style="margin-top:4px;color:#666;">
									<?php echo esc_html( sprintf( __( 'Auto-skips when a %s HowTo block is present in the post content.', 'rankready-ai-llm-seo' ), $seo_plugin ) ); ?>
								</p>
							<?php endif; ?>
						</td>
					</tr>
	...
	private static function render_tab_llm(): void {
		...
		$rnrd_status_signals = array(
			...
			array(
				'label' => __( 'WebMCP manifest', 'rankready-ai-llm-seo' ),
				'on'    => 'on' === get_option( RNRD_OPT_MCP_ENABLE, 'on' ),
		...
		?>
		<div class="rnrd-card rnrd-agent-status" style="margin-bottom:24px;background:linear-gradient(135deg,var(--rnrd-color-surface,#fff) 0%,var(--rnrd-color-brand-soft,#f0f6fc) 100%);border:1px solid var(--rnrd-color-border,#c3c4c7);">
			<div style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:14px;">
				<div>
					<h2 class="rnrd-card-title" style="margin:0;"><?php esc_html_e( 'Agent Visibility', 'rankready-ai-llm-seo' ); ?></h2>
					<p class="rnrd-card-goal" style="margin:4px 0 0;"><?php esc_html_e( 'Coverage check — how many AI-readable signals are active on your site.', 'rankready-ai-llm-seo' ); ?></p>
					<p class="rnrd-card-desc" style="margin:4px 0 0;">
						<?php
						printf(
				...
				<div style="text-align:right;min-width:120px;">
					<div style="font-size:32px;font-weight:700;line-height:1;color:var(--rnrd-color-brand,#2271b1);">
						<?php echo esc_html( $rnrd_status_pct ); ?>%
					</div>
					<div style="font-size:11px;color:var(--rnrd-color-text-muted,#646970);text-transform:uppercase;letter-spacing:0.04em;margin-top:2px;">
						<?php esc_html_e( 'Coverage', 'rankready-ai-llm-seo' ); ?>
					</div>
				</div>
			</div>
			...
			<ul style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:6px 18px;margin:0;padding:0;list-style:none;font-size:12px;">
				<?php foreach ( $rnrd_status_signals as $sig ) :
					$icon  = $sig['on'] ? '✓' : '○';
					$color = $sig['on'] ? 'var(--rnrd-color-success,#00a32a)' : 'var(--rnrd-color-text-muted,#646970)';
					?>
					<li style="display:flex;align-items:center;gap:6px;">
						...
						<span style="color:<?php echo $sig['on'] ? 'var(--rnrd-color-ink,#1d2327)' : 'var(--rnrd-color-text-muted,#646970)'; ?>;">
							<?php echo esc_html( $sig['label'] ); ?>
						</span>
						<?php if ( $sig['on'] && ! empty( $sig['url'] ) ) : ?>
							<a href="<?php echo esc_url( $sig['url'] ); ?>" target="_blank" rel="noopener" style="font-size:11px;text-decoration:none;color:var(--rnrd-color-brand,#2271b1);">↗</a>
						<?php endif; ?>
					</li>
				<?php endforeach; ?>
			</ul>
		...
		<form method="post" action="options.php" novalidate="novalidate" class="rnrd-brand-form">
			...
			<div class="rnrd-card" style="margin-bottom:24px;border:1px solid var(--rnrd-color-border,#c3c4c7);">
				<h2 class="rnrd-card-title" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
					...
					<span style="font-size:11px;background:var(--rnrd-color-brand-soft,#f0f6fc);color:var(--rnrd-color-info-text,#135e96);padding:2px 8px;border-radius:9999px;font-weight:600;">
						<?php esc_html_e( 'SINGLE SOURCE OF TRUTH', 'rankready-ai-llm-seo' ); ?>
					</span>
					<?php if ( $rnrd_brand_complete ) : ?>
						<span style="font-size:11px;background:var(--rnrd-color-success-bg,#d1ecdf);color:var(--rnrd-color-success-text,#0a6c39);padding:2px 8px;border-radius:9999px;font-weight:600;">✓ <?php esc_html_e( 'Complete', 'rankready-ai-llm-seo' ); ?></span>
					<?php else : ?>
						<span style="font-size:11px;background:var(--rnrd-color-warning-bg,#fcf9e8);color:var(--rnrd-color-warning-text,#674c00);padding:2px 8px;border-radius:9999px;font-weight:600;">⚠ <?php esc_html_e( 'Incomplete', 'rankready-ai-llm-seo' ); ?></span>
					<?php endif; ?>
				</h2>
				<p class="rnrd-card-goal"><?php esc_html_e( 'Tell AI engines who you are — once. Powers llms.txt, robots.txt comment, FAQ prompt, AI summary prompt, MCP abilities, homepage markdown.', 'rankready-ai-llm-seo' ); ?></p>
					...
					<tr>
						...
						<td>
							<textarea id="rnrd_brand_terms"
									  ...
									  rows="4"
									  class="large-text"
									  placeholder="<?php esc_attr_e( "One name per line. e.g.\nThe Plus Addons for Elementor\nNexterWP\nYour Brand Name", 'rankready-ai-llm-seo' ); ?>"
							><?php echo esc_textarea( $rnrd_brand_terms_raw ); ?></textarea>
							<p class="description"><?php esc_html_e( 'Use the exact capitalisation and spacing you want AI engines to use. Each line is one canonical name — variants like "theplusaddons" vs "The Plus Addons for Elementor" become a single recognised entity.', 'rankready-ai-llm-seo' ); ?></p>
						</td>
					</tr>
				</table>
		...
		<!-- ── AI Crawler Access Log — moved to Insights ───────────────── -->
		<div class="rnrd-card" style="margin-bottom:24px;">
			<h2 class="rnrd-card-title"><?php esc_html_e( 'AI Crawler Access Log', 'rankready-ai-llm-seo' ); ?></h2>
			<p class="rnrd-card-desc">
				<?php esc_html_e( 'Live tracking of every known AI bot reading your llms.txt, Markdown, and homepage endpoints — citation bots (ChatGPT-User, OAI-SearchBot, PerplexityBot, Claude-Web) vs training bots (GPTBot, Google-Extended, ClaudeBot) — now lives on a dedicated tab so this view stays focused on crawler controls.', 'rankready-ai-llm-seo' ); ?>
			</p>
			<p style="margin:14px 0 0;">
				<a class="button button-primary" href="<?php echo esc_url( admin_url( 'admin.php?page=' . self::MENU_SLUG . '&tab=insights&sub=bot-activity' ) ); ?>">
					<?php esc_html_e( 'View bot activity in Insights', 'rankready-ai-llm-seo' ); ?>
				</a>
			</p>
		</div>
		...
		<form method="post" action="options.php" novalidate="novalidate">
			...
			<?php
			$rnrd_referral_enable = (string) get_option( RNRD_OPT_AI_REFERRAL_ENABLE, 'on' );
			$rnrd_ref_counts      = class_exists( 'RNRD_AI_Referral' ) ? RNRD_AI_Referral::aggregate_last_n_days( 30 ) : array();
			$rnrd_ref_total       = (int) array_sum( $rnrd_ref_counts );
			?>
			<div class="rnrd-card" style="margin-bottom:24px;">
				<h2 class="rnrd-card-title"><?php esc_html_e( 'AI Referral Traffic', 'rankready-ai-llm-seo' ); ?></h2>
				<p class="rnrd-card-goal"><?php esc_html_e( 'Count visits from chatgpt.com, perplexity.ai, claude.ai, gemini.google.com, copilot.microsoft.com — privacy-respecting.', 'rankready-ai-llm-seo' ); ?></p>
				...
				<table class="form-table rnrd-form-table">
					<tr>
						...
						<td>
							<label class="rnrd-toggle">
								<input type="checkbox" name="<?php echo esc_attr( RNRD_OPT_AI_REFERRAL_ENABLE ); ?>" value="on" <?php checked( $rnrd_referral_enable, 'on' ); ?> />
								<span><?php esc_html_e( 'Count visits from AI engines', 'rankready-ai-llm-seo' ); ?></span>
							</label>
							<p class="description"><?php esc_html_e( 'Detects Referer header values from chatgpt.com, perplexity.ai, gemini.google.com, claude.ai, copilot.microsoft.com.', 'rankready-ai-llm-seo' ); ?></p>
						</td>
					</tr>
				</table>

				<?php if ( 'on' !== $rnrd_referral_enable ) :
					self::render_locked_preview( array(
						'option_key'   => 'rnrd_ai_referral_enable',
						'enable_label' => __( 'Enable AI Referral Tracking', 'rankready-ai-llm-seo' ),
						'bullets'      => array(
							__( 'Count visits from chatgpt.com, perplexity.ai, claude.ai, gemini.google.com, copilot.microsoft.com', 'rankready-ai-llm-seo' ),
							__( 'Privacy-respecting — Sec-GPC + DNT headers honored', 'rankready-ai-llm-seo' ),
							__( 'Race-safe counter with shutdown-batched DB writes', 'rankready-ai-llm-seo' ),
							__( 'Insights → AI Referral Traffic dashboard', 'rankready-ai-llm-seo' ),
						),
					) );
				endif; ?>

				<?php if ( 'on' === $rnrd_referral_enable && $rnrd_ref_total > 0 ) : ?>
					<div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:8px;">
						<?php foreach ( $rnrd_ref_counts as $source => $count ) :
							if ( $count <= 0 ) continue;
							$label = class_exists( 'RNRD_AI_Referral' ) ? RNRD_AI_Referral::source_label( $source ) : ucfirst( $source );
							?>
							<div style="flex:1;min-width:120px;padding:10px 12px;background:var(--rnrd-color-surface-2,#f6f7f7);border-radius:var(--rnrd-radius-md,6px);">
								<div style="font-size:11px;color:var(--rnrd-color-text-muted,#646970);text-transform:uppercase;letter-spacing:.04em;"><?php echo esc_html( $label ); ?></div>
								<div style="font-size:20px;font-weight:600;margin-top:2px;"><?php echo esc_html( number_format_i18n( $count ) ); ?></div>
							</div>
						<?php endforeach; ?>
					</div>
					<p style="margin-top:10px;font-size:11px;color:var(--rnrd-color-text-muted,#646970);">
						<?php
						printf(
							esc_html__( '%s total AI-sourced visits in the last 30 days.', 'rankready-ai-llm-seo' ),
							'<strong>' . esc_html( number_format_i18n( $rnrd_ref_total ) ) . '</strong>'
						);
						?>
					</p>
				<?php elseif ( 'on' === $rnrd_referral_enable ) : ?>
					<p style="margin-top:8px;font-size:12px;color:var(--rnrd-color-text-muted,#646970);">
						<?php esc_html_e( 'No AI-sourced visits in the last 30 days yet. Counters update on every front-end pageview that arrives with an AI Referer header. Typical first citation: 2–6 weeks.', 'rankready-ai-llm-seo' ); ?>
					</p>
				<?php endif; ?>
			</div>
			<!-- ── /AI Referral Traffic ───────────────────────────────────────── -->
			...
			<p class="description" style="margin:0 0 16px;padding:10px 12px;background:var(--rnrd-color-surface-2,#f6f7f7);border-radius:var(--rnrd-radius-md,6px);font-size:12px;color:var(--rnrd-color-text-muted,#646970);">
				<?php esc_html_e( 'Looking for Brand Terms? It now lives in the unified Brand Identity card at the top of this tab — same field, same data, just grouped with site name / summary / about so you can fill them once and use them everywhere.', 'rankready-ai-llm-seo' ); ?>
			</p>

			<?php submit_button( __( 'Save LLM Settings', 'rankready-ai-llm-seo' ) ); ?>
		</form>
	...
	private static function render_tab_faq(): void {
		?>
			...
			<div class="rnrd-card">
				...
				<p class="rnrd-card-goal"><?php esc_html_e( 'Discover real user questions + answer with AI. Outputs FAQPage schema that AI engines preferentially cite.', 'rankready-ai-llm-seo' ); ?></p>
				...
				<table class="form-table rnrd-form-table">
					...
					<tr>
						...
						<td>
							<textarea id="rnrd_faq_brand_terms" name="<?php echo esc_attr( RNRD_OPT_FAQ_BRAND_TERMS ); ?>"
							          ...
							          placeholder="<?php esc_attr_e( 'e.g. Acme Plugin, Your Brand Name, Your Product Name (one per line or comma-separated)', 'rankready-ai-llm-seo' ); ?>"
					...
					<tr>
						<th scope="row"><?php esc_html_e( 'Auto-Generate on Publish', 'rankready-ai-llm-seo' ); ?></th>
						<td>
							<?php $faq_auto_gen = (string) get_option( RNRD_OPT_FAQ_AUTO_GENERATE, 'off' ); ?>
							<label>
				...
				<?php if ( ! ( function_exists( 'rnrd_is_pro' ) && rnrd_is_pro() ) ) : ?>
				<?php self::render_pro_gate(
					__( 'Auto-Generate FAQ on Publish', 'rankready-ai-llm-seo' ),
					__( 'Auto-generate FAQ schema the moment you hit Publish. Coming Soon.', 'rankready-ai-llm-seo' )
				); ?>
				<?php endif; ?>
				...
				<table class="form-table rnrd-form-table">
					<tr>
						...
						<td>
							...
							<label>
								<input type="radio" name="<?php echo esc_attr( RNRD_OPT_FAQ_AUTO_DISPLAY ); ?>" value="on" <?php checked( $faq_auto, 'on' ); ?> />
								<?php esc_html_e( 'On — Automatically inject FAQ into post content', 'rankready-ai-llm-seo' ); ?>
							</label><br/>
							<label>
								<input type="radio" name="<?php echo esc_attr( RNRD_OPT_FAQ_AUTO_DISPLAY ); ?>" value="off" <?php checked( $faq_auto, 'off' ); ?> />
								<?php esc_html_e( 'Off — Only show via block, widget, or shortcode', 'rankready-ai-llm-seo' ); ?>
							</label>
						</td>
					</tr>
	...
	private static function render_tab_tools(): void {
		...
		self::render_card_branding();   // rc.11 — Branding placeholder (Coming Soon).
	...
	private static function render_card_bulk_summary(): void {
		...
		<div class="rnrd-card">
			<h2 class="rnrd-card-title"><?php esc_html_e( 'Bulk Regenerate — AI Summaries', 'rankready-ai-llm-seo' ); ?></h2>
			...
			<table class="form-table rnrd-form-table" style="width:auto;">
				<tr>
					...
					<td>
						<?php foreach ( $post_types as $slug => $label ) : ?>
							<label style="display:block;margin-bottom:4px;">
								<input type="checkbox" class="rnrd-bulk-type" value="<?php echo esc_attr( $slug ); ?>" checked />
								<?php echo esc_html( $label ); ?>
							</label>
						<?php endforeach; ?>
					</td>
				...
				<tr>
					<th></th>
					<td>
						<button id="rnrd-bulk-start" class="button button-primary"><?php esc_html_e( 'Start Bulk Generate', 'rankready-ai-llm-seo' ); ?></button>
						<button id="rnrd-bulk-resume" class="button button-secondary" style="margin-left:8px;"><?php esc_html_e( 'Resume', 'rankready-ai-llm-seo' ); ?></button>
						...
						<p class="description" style="margin-top:4px;"><?php esc_html_e( 'Resume picks up from where you stopped.', 'rankready-ai-llm-seo' ); ?></p>
					</td>
				</tr>
			</table>
		...
		<?php
			self::render_pro_gate(
				...
				__( 'Generate AI summaries across all existing published posts in one run. Bulk processing is Coming Soon. Manual summaries remain unlimited.', 'rankready-ai-llm-seo' )
			);
		?>
	...
	private static function render_card_bulk_faq(): void {
		...
		// Free build: show only the Coming Soon gate. The underlying bulk UI
		// is implemented but intentionally hidden until activation.
		if ( ! $tools_is_pro ) {
			self::render_pro_gate(
				__( 'Bulk Regenerate — FAQ', 'rankready-ai-llm-seo' ),
				__( 'Generate FAQ schema across all existing published posts in one run. Bulk processing is Coming Soon. Manual FAQ generation remains unlimited.', 'rankready-ai-llm-seo' )
			);
			return;
		}
		?>
		...
		<div class="rnrd-card">
			<h2 class="rnrd-card-title"><?php esc_html_e( 'Bulk Generate FAQs', 'rankready-ai-llm-seo' ); ?></h2>
			...
			<table class="form-table rnrd-form-table" style="width:auto;">
				<tr>
					...
					<td>
						<?php foreach ( $post_types as $slug => $label ) : ?>
							<label style="display:block;margin-bottom:4px;">
								<input type="checkbox" class="rnrd-faq-bulk-type" value="<?php echo esc_attr( $slug ); ?>" checked />
								<?php echo esc_html( $label ); ?>
							</label>
						<?php endforeach; ?>
					</td>
				...
				<tr>
					<th></th>
					<td>
						<button id="rnrd-faq-bulk-start" class="button button-primary"><?php esc_html_e( 'Start Bulk FAQ Generate', 'rankready-ai-llm-seo' ); ?></button>
						<button id="rnrd-faq-bulk-resume" class="button button-secondary" style="margin-left:8px;"><?php esc_html_e( 'Resume', 'rankready-ai-llm-seo' ); ?></button>
						...
						<p class="description" style="margin-top:4px;"><?php esc_html_e( 'Skips posts with unchanged content. Resume picks up from where you stopped.', 'rankready-ai-llm-seo' ); ?></p>
					</td>
				</tr>
			</table>
	...
	private static function render_card_api_usage(): void {
		...
		?>
		<div class="rnrd-card">
			<h2 class="rnrd-card-title"><?php esc_html_e( 'Cost & Tokens Burned', 'rankready-ai-llm-seo' ); ?></h2>
			<p class="rnrd-card-goal"><?php esc_html_e( 'How much you\'ve spent on AI generations. Estimated from token counts × blended provider rates.', 'rankready-ai-llm-seo' ); ?></p>
			<p class="rnrd-card-desc"><?php esc_html_e( 'Cumulative API usage tracked since this feature was enabled.', 'rankready-ai-llm-seo' ); ?></p>
			...
			<div style="margin-top:12px;margin-bottom:8px;display:flex;gap:12px;flex-wrap:wrap;">
				<div style="padding:10px 16px;background:#f0f6fc;border:1px solid #c8d8e8;border-radius:6px;display:inline-block;">
					...
					<span style="font-size:13px;color:#646970;margin-left:6px;"><?php esc_html_e( 'Estimated Cost (blended)', 'rankready-ai-llm-seo' ); ?></span>
				</div>
				...
				<div style="padding:10px 16px;background:#fef8f0;border:1px solid #e8d8c0;border-radius:6px;display:inline-block;">
					...
					<span style="font-size:13px;color:#646970;margin-left:6px;"><?php esc_html_e( 'DataForSEO Cost', 'rankready-ai-llm-seo' ); ?></span>
				</div>
				<?php endif; ?>
			</div>
			...
			<h3 style="font-size:14px;margin:16px 0 8px;color:#1d2327;"><?php
				echo esc_html( sprintf(
					...
					__( 'AI Provider — %s', 'rankready-ai-llm-seo' ),
					RNRD_LLM::get_provider_label( RNRD_LLM::get_active_provider() )
				) );
			?></h3>
			<div class="rnrd-stats-row">
				<div class="rnrd-stat">
					<span class="rnrd-stat-number"><?php echo esc_html( number_format_i18n( $summary_tokens ) ); ?></span>
					<span class="rnrd-stat-label"><?php esc_html_e( 'Summary Tokens', 'rankready-ai-llm-seo' ); ?></span>
				</div>
				<div class="rnrd-stat">
					<span class="rnrd-stat-number"><?php echo esc_html( number_format_i18n( $faq_tokens ) ); ?></span>
					<span class="rnrd-stat-label"><?php esc_html_e( 'FAQ Tokens', 'rankready-ai-llm-seo' ); ?></span>
				</div>
				<div class="rnrd-stat">
					<span class="rnrd-stat-number"><?php echo esc_html( number_format_i18n( $total_tokens ) ); ?></span>
					<span class="rnrd-stat-label"><?php esc_html_e( 'Total Tokens', 'rankready-ai-llm-seo' ); ?></span>
				</div>
				<div class="rnrd-stat">
					<span class="rnrd-stat-number"><?php echo esc_html( number_format_i18n( $total_calls ) ); ?></span>
					<span class="rnrd-stat-label"><?php esc_html_e( 'API Calls', 'rankready-ai-llm-seo' ); ?></span>
				</div>
			</div>

			<h3 style="font-size:14px;margin:16px 0 8px;color:#1d2327;"><?php esc_html_e( 'DataForSEO', 'rankready-ai-llm-seo' ); ?></h3>
			<div class="rnrd-stats-row">
				<div class="rnrd-stat">
					<span class="rnrd-stat-number"><?php echo esc_html( number_format_i18n( $dfs_calls ) ); ?></span>
					<span class="rnrd-stat-label"><?php esc_html_e( 'API Calls', 'rankready-ai-llm-seo' ); ?></span>
				</div>
				<div class="rnrd-stat">
					<span class="rnrd-stat-number">$<?php echo esc_html( number_format( $dfs_cost, 4 ) ); ?></span>
					<span class="rnrd-stat-label"><?php esc_html_e( 'Total Cost', 'rankready-ai-llm-seo' ); ?></span>
				</div>
			</div>
		...
		<div class="rnrd-card" id="rnrd-diagnostics-card">
			...
			<div id="rnrd-diag-copy-row" style="display:none;margin-top:14px;padding-top:14px;border-top:1px solid #e5e5e5;">
				<p style="margin:0 0 6px;font-size:13px;color:#1d2327;">
					...
					<?php esc_html_e( 'Click below to copy a full diagnostic report with active plugins, versions, and conflict details — then paste into our Discord or support email.', 'rankready-ai-llm-seo' ); ?>
				</p>
	...
	public static function render_meta_box( $post ): void {
		...
		?>
		<style>
			.rnrd-mb { font-size: var(--rnrd-text-md, 13px); }
			.rnrd-mb__status {
				display: flex; align-items: flex-start; gap: 8px;
				padding: 10px 12px;
				margin: -6px -12px 10px;
				border-radius: 0;
				font-size: var(--rnrd-text-md, 13px);
				line-height: 1.4;
			}
			.rnrd-mb__status--ok    { background: var(--rnrd-color-success-bg, #d1ecdf); color: var(--rnrd-color-success-text, #0a6c39); }
			.rnrd-mb__status--warn  { background: var(--rnrd-color-warning-bg, #fcf9e8); color: var(--rnrd-color-warning-text, #674c00); }
			.rnrd-mb__status--muted { background: var(--rnrd-color-surface-2, #f6f7f7); color: var(--rnrd-color-text-muted, #646970); }
			.rnrd-mb__icon { font-size: 16px; line-height: 1.2; flex-shrink: 0; }
			.rnrd-mb__title { font-weight: var(--rnrd-weight-semibold, 600); margin-bottom: 2px; }
			.rnrd-mb__sub { font-size: var(--rnrd-text-sm, 12px); opacity: 0.85; }
			.rnrd-mb__preview {
				margin: 0 0 10px;
				padding: 8px 10px;
				background: var(--rnrd-color-surface-2, #f6f7f7);
				border-radius: var(--rnrd-radius-md, 6px);
				font-size: var(--rnrd-text-sm, 12px);
				color: var(--rnrd-color-ink-soft, #3c434a);
			}
			.rnrd-mb__preview ul { margin: 0; padding-left: 14px; list-style: disc; }
			.rnrd-mb__preview li { margin-bottom: 3px; }
			.rnrd-mb details { margin-top: 8px; }
			.rnrd-mb summary {
				cursor: pointer;
				font-size: var(--rnrd-text-sm, 12px);
				color: var(--rnrd-color-text-muted, #646970);
				padding: 4px 0;
				user-select: none;
			}
			.rnrd-mb summary:hover { color: var(--rnrd-color-brand, #2271b1); }
			.rnrd-mb__advanced { padding-top: 8px; }
			.rnrd-mb__field { margin: 0 0 10px; }
			.rnrd-mb__field-label {
				display: block;
				font-size: var(--rnrd-text-xs, 11px);
				font-weight: var(--rnrd-weight-semibold, 600);
				text-transform: uppercase;
				letter-spacing: 0.04em;
				color: var(--rnrd-color-text-muted, #646970);
				margin-bottom: 4px;
			}
			.rnrd-mb__field select { width: 100%; }
			.rnrd-mb__hint { margin: 2px 0 0; font-size: var(--rnrd-text-xs, 11px); color: var(--rnrd-color-text-muted, #646970); }
		</style>
